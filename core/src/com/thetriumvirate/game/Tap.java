...
public class Tap extends InputAdapter{
	...
	public boolean checkAllClicked(int mouseX, int mouseY) {
		return mouseX >= pos_x && mouseX <= pos_x + PLANK_WIDTH && mouseY >= pos_y && mouseY <= pos_y + TAP_OFFSET_Y + TAP_HEIGHT;
	}
	
	public boolean checkTapClick(int mouseX, int mouseY) {
		return mouseX >= this.pos_x + TAP_OFFSET_X && mouseX <= this.pos_x + TAP_OFFSET_X + TAP_WIDTH && mouseY >= this.pos_y + TAP_OFFSET_Y && mouseY <= this.pos_y + TAP_OFFSET_Y + TAP_HEIGHT;
	}
